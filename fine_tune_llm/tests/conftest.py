...
def mock_ensemble_system():
    ...
    mock = Mock()
    mock.voters = {"regex_dsl": Mock(), "tfidf_lr": Mock(), "llm_lora": Mock()}
    mock.predict_with_cascade = Mock(
        return_value={"decision": "HIGH_RISK", "confidence": 0.8, "abstain": False, "tier": 1}
    )
    return mock
...
def generate_drift_data(
    reference_size: int = 1000, drift_size: int = 100, drift_magnitude: float = 0.5
